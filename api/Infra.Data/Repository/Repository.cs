...
namespace Infra.Data.Repository
{
  public class Repository : IRepository
  {
    ...
    {
      var lsta = await db.GetCollection<T>(collectionName).FindAsync("{}");
    ...
    public Task<IQueryable<T>> Query<T>() where T : class =>
      Task.FromResult((IQueryable<T>)db.GetCollection<T>(typeof(T).Name).AsQueryable());

    public Task<IQueryable<T>> Query<T>(string collectionName) where T : class =>
      Task.FromResult((IQueryable<T>)db.GetCollection<T>(collectionName).AsQueryable());
  }
}

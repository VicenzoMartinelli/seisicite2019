...
import { CssBaseline, Container, Paper, Divider, FormControl, InputLabel, Select, Input, MenuItem } from '@material-ui/core';
...
function Report() {
  ...
  useEffect(() => {
    async function loadData() {
      ...
      findModalidades(event, modalidade)
        .then(res => {
  ...
  useEffect(() => {
    async function loadData() {
      ...
      findReportArticles(event, modalidade)
        .then(res => {
          setData(res.data);
          setOpering(false);
        });
        ...
        <Container className={classes.main} maxWidth={false}>
          <Paper>
            <div style={{ paddingTop: '20px', paddingBottom: '20px' }}>
              <FormControl style={{ width: '30%', marginLeft: '10%', marginRight: '10%' }}>
                <InputLabel className={classes.label} htmlFor="event">Evento</InputLabel>
                <Select
                  value={event}
              ...
              <FormControl style={{ width: '30%', marginLeft: '10%', marginRight: '10%' }}>
                <InputLabel className={classes.label} htmlFor="modality">Modality</InputLabel>
                <Select
                  value={modalidade}

...
import {
  Dialog, AppBar, Toolbar, Typography, Slide, IconButton, Select, FormControl, InputLabel, MenuItem, Grid, Input,
  FormLabel, RadioGroup, FormControlLabel, Radio, Box, FormHelperText, Divider
...
const schema = yup.object().shape({
  title: yup
    .string()
    .required('Informe um título válido'),
  resume: yup
    .string()
    .required('Informe um resumo válido'),
  building: yup
    .string()
    .required('Informe um local válido'),
  room: yup
    .string()
    .required('Informe uma sala válida'),
  evaluatorId: yup
    .string()
    .required("Informe ao menos um avaliador"),
  evaluator2Id: yup
    .string()
...
export default function ModalArticleEdit({
  ...
  return (
    <Dialog fullScreen open={editOpened} onClose={handleClose} TransitionComponent={Transition}>
            ...
            return (
              <form onSubmit={handleSubmit}>
                ...
                <FormControl fullWidth={true}>
                  ...
                  <FormHelperText error={(errors.evaluatorId && touched.evaluatorId)}>
                    {(errors.evaluatorId && touched.evaluatorId) && errors.evaluatorId}
                  </FormHelperText>
                </FormControl>
                ...
                <FormControl fullWidth={true} className={classes.formControlMargin} >
                  ...
                  <Select
                    ...
                    {[{ id: null, name: null }].concat(dataAvaliadores).map((x) => (
                      <MenuItem key={x.id} value={x.id}>{x.name}</MenuItem>
                    ))}
                  </Select>
                  <FormHelperText error={(errors.evaluator2Id && touched.evaluator2Id)}>
                    {(errors.evaluator2Id && touched.evaluator2Id) && errors.evaluator2Id}
                  </FormHelperText>
                </FormControl>
                ...
                <Box className={classes.footerForm}>
                  <Button disabled={savingOperation} color="primary" className={classes.marginTopElement} round onClick={submitForm}>
                    {!savingOperation && <><Check /> Salvar</>}
                    {savingOperation && <><RefreshOutlined className={classes.animationSpin} /> Salvar</>}
                  </Button>
                </Box>
              </form>
            );

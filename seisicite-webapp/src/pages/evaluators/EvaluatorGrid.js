...
import { approve, cancelEvaluators, changeEvent } from "../../services/api";
...
export default function EvaluatorGrid({
  toApprove,
  rows,
  setRows,
  addToast,
  ...
  function handleChangeEvent(id, evento) {
    setOpering(true);

    changeEvent(id, evento)
      .then(async res => {
        if (res.success !== undefined && !res.success) {
          addToast(res.msg, { appearance: "error", autoDismiss: true });
          setOpering(false);
                ...
                return (
                  ...
                  >
                    ...
                    <TableCell align="left">
                      {row.attendedModalities.join(", ")}
                    </TableCell>
                    <TableCell align="left">
                      <Checkbox
                        checked={row.isSei}
                        onChange={e => {
                          handleChangeEvent(row.id, 1);
                        }}
                        color="primary"
